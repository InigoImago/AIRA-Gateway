...
MUTATIONS = [
    ...
    Mutation(
        "ID42",
        "a credential covers both transports, so the gRPC leg is not left anonymous",
    ...
    Mutation(
        "HEC1",
        "the delivery channel's metrics go to the HEC metrics exporter, not the events index",
        "deploy/compose/otel/collector-forward-splunk-hec.yaml",
    ...
    Mutation(
        "HEC2",
        "the observability channel over HEC keeps debug and file/arrived on its pipelines",
        "deploy/compose/otel/collector-backend-splunk-hec.yaml",
        "      exporters: [splunk_hec/backend_metrics, debug, file/arrived]",
        "      exporters: [splunk_hec/backend_metrics]",
        "tools/tests/test_splunk_hec_is_a_transport_on_either_channel.py",
    ),
    Mutation(
        "HEC3",
        "the HEC metrics exporter writes to the metrics index",
        "deploy/compose/otel/collector-backend-splunk-hec.yaml",
    ...
    Mutation(
        "HEC4",
        "a forgotten HEC token is sent as the name of its variable, not as nothing",
        "deploy/compose/docker-compose.yml",
        "      AIRA_OTEL_FORWARD_HEC_TOKEN: ${AIRA_OTEL_FORWARD_HEC_TOKEN:-AIRA_OTEL_FORWARD_HEC_TOKEN-is-not-set}",
        "      AIRA_OTEL_FORWARD_HEC_TOKEN: ${AIRA_OTEL_FORWARD_HEC_TOKEN:-}",
        "tools/tests/test_splunk_hec_is_a_transport_on_either_channel.py",
    ),
    Mutation(
        "HEC5",
        "the delivery channel's HEC leg reads no variable of the observability channel",
        "deploy/compose/otel/collector-forward-splunk-hec.yaml",
