...
COMPOSE_ONLY = {
    ...
    # The rest of the observability channel's core (`FRD-620`, `ADR-0024`): the delivery channel's
    # names one prefix along, except the transport each does not default to (`_HTTP_ENDPOINT` here,
    # `_GRPC_ENDPOINT` there).
    ...
    # The standing-in SIEM (`debug` profile): where it is published, and how many batches it
    # keeps. Read by `tools/otlp_inspector.py`, which is a debugging tool rather than a plane.
    "AIRA_PUBLISH_OTLP_INSPECTOR_PORT",
