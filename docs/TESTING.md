...
- Enforced: `ruff`, `ruff format`, `mypy --strict`, `prettier`, and a coverage **floor** — not the
  ...
  | | gate | last measured |
  | --- | --- | --- |
  | Python (`--cov-fail-under`) | **90%** | 95.83% |
  | Frontend statements | 90% | 93.19% |
  | Frontend branches | **92%** | 92.15% |
  | Frontend lines | 93% | 94.88% |
  | Frontend functions | 75% | 76.39% |
...
to notice. **735 properties**; the figure is stated in `CLAUDE.md` and a test
