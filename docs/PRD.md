...
| 13 | Zugelassene Modelle je Use Case | **teilweise** — `allow_check` done, Fähigkeiten durchgesetzt (`FRD-114` done), **Freigabe durchgesetzt** (`FRD-307` done 2026-08-09: `approved` ist eine Dispatch-Bedingung an jedem Hop). Offen ist die **Auswahl**: der Pipeline-Builder nimmt Modellnamen weiterhin als Freitext, kennt den Katalog nicht und bietet damit an, was der Server ablehnt | `FRD-300` done, `FRD-114` done, `FRD-307` Freigabe done / Picker offen |
