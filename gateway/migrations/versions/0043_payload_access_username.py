...
def upgrade() -> None:
    ...
    `payload_accesses` is the record `ADR-0009` reopened this view on: reading somebody's stored
    ...
    """
    op.add_column("payload_accesses", sa.Column("username", sa.String(length=255), nullable=True))
    ...
    op.create_index("ix_payload_accesses_username", "payload_accesses", ["username"])


def downgrade() -> None:
    op.drop_index("ix_payload_accesses_username", table_name="payload_accesses")
    op.drop_column("payload_accesses", "username")
