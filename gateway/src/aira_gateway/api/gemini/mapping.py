"""Gemini ⇄ canonical mappers (FRD-100). Pure functions, free of FastAPI."""
...
def _canonical_parts(content: schemas.Content, limits: Limits, offset: int) -> list[CanonicalPart]:
    """Map one content's parts, preserving their order ("this image, then this question" is a
    different prompt from the reverse).

    Every attachment check runs here, at the surface, so nothing downstream ever holds a part that
    failed one and no adapter has to repeat it.
    """
    ...
    for offset_index, part in enumerate(content.parts):
        ...
        if part.functionCall is not None:
            # Google matches a result to its call by name and sends no id; other dialects need one.
            # Deterministic, so a client echoing it back still matches.
        ...
        if part.functionResponse is not None:
            ...
            parts.append(
                ToolResultPart(
                    ...
                    name=result.name,
                    # Serialised once, here: dialects disagree on string vs. object, and the round
                    # trip has to be stable.
...
def gemini_to_canonical(
    ...
    # Across the whole request: one document split over five messages is still one large request.
    ...
    return CanonicalRequest(
        ...
        # Carried, never dropped (`FRD-124`): the dialect either expresses each of these or the
        ...
        # Function parameters are caller-supplied structure too, so they get the schema parser
        # and its bounds (`FRD-131`).
        tools=tuple(
            ToolDeclaration(
...
def gemini_to_embedding(
    ...
    - an attachment — embedding a document means chunking it, which is the consumer's decision;
    - a batch mixing task types or dimensionalities — one call is metered, validated and recorded
    ...
    for index, entry in enumerate(requests):
        ...
        # Joined with nothing between them, because that is how Google combines a multi-part
        # content (measured, `FRD-113` §11 — not a centroid). A separator would quietly change the
        # vector; a caller who wants a centroid computes it from `batchEmbedContents`.
...
def chunk_to_gemini(chunk: CanonicalChunk, model: str) -> schemas.GenerateContentResponse:
    """One streamed chunk as a Gemini response.

    A chunk carries a text delta, completed tool calls, or both. Calls arrive whole on the chunk
    that ends the message (`FRD-131` FR-6) — half a function call is not a smaller one.
    """
    usage = chunk.usage
    parts: list[schemas.Part] = []
...
def upstream_model_to_gemini(model: UpstreamModel) -> schemas.GeminiModel:
    ...
    return schemas.GeminiModel(
        ...
        # Where the model is reached, as the adapter was configured (`FRD-507` FR-1). Blank stays
        # blank: an adapter that names no region has made no claim about one.
