"""The question catalogue's content (`FRD-504`): a hundred questions in one flat list.

Every model is asked all of them, so "this model is better" is a statement about the models rather
than about which questions somebody picked. Each `expectation` says *what to look at*, not the
correct answer — it is shown to the person rating and never matched against, and a rater told the
answer stops reading the one in front of them.

Seeded, and then owned by IT Security in the console.
"""
