...
describe('MultiSelect', () => {
  ...
  it('stays recognisably a control when it is read-only, greyed rather than gone', () => {
    /** **Inert, not re-drawn** (`FRD-206`): a row of chips without a field reads as prose, not as
     *  a control. A disabled input in the same place says "this is the control and it is not yours
     *  to use". What must still be gone is anything that *acts*: the per-chip remove, and the list
     *  toggle. */
