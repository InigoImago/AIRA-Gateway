...
test.describe('Model release', () => {
  ...
  test('releases a model and takes it away again', async ({ page }) => {
    ...
    // **Named, not counted.** This used to arithmetic its way from "how many chips are there" to
    // "what does one keypress do", which holds only when the highlight happens to land on a model
    // that is already chosen — and clicking the field *opens* the list, so ArrowDown **moves**
    ...
    const toggle = async () => {
      ...
      // Escape deliberately: the list floats over what follows and stays open so several models
      // can be picked in one go, so a reader closes it before reaching for Save. That is the
      // combobox contract, and asserting it here is what keeps Escape working.
      await search.press('Escape');
    };

    await toggle();
    await expect(chip).toHaveCount(wasChosen ? 0 : 1);
    ...
    await toggle();
    await expect(chip).toHaveCount(wasChosen ? 1 : 0);
    await page.getByTestId('save-release').click();
